    ...
    >
      ...
      >
        ...
        <div style={{ marginTop: spacing.lg }}>
          {step === 1 && <Step1Welcome />}
          {step === 2 && (
            <Step2RoleIndustry role={role} setRole={setRole} industry={industry} setIndustry={setIndustry} />
          )}
          {step === 3 && (
            <Step3PainPoints
          ...
          {step === 5 && <Step5Sensitivity sensitiveData={sensitiveData} setSensitiveData={setSensitiveData} />}
          {step === 6 && (
            <Step6Summary
              role={role!}
              industry={industry}
...
function Step1Welcome() {
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <p style={{ fontSize: typography.fontSize.lg, color: colors.neutral[600], marginBottom: spacing.xl }}>
...
function Step2RoleIndustry({
  ...
  return (
    <div>
      <h2 style={{ fontSize: typography.fontSize['2xl'], fontWeight: typography.fontWeight.semibold, marginBottom: spacing.lg }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: spacing.md, marginBottom: spacing.xl }}>
        {ROLES.map((r) => (
          ...
          >
            ...
            <div style={{ fontSize: typography.fontSize.sm, fontWeight: typography.fontWeight.medium }}>{r.label}</div>
          </div>
        ))}
      </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: spacing.md }}>
        {PAIN_POINTS.map((pain) => (
          <div
            key={pain.value}
            onClick={() => setTags(tags.includes(pain.value) ? tags.filter((t) => t !== pain.value) : [...tags, pain.value])}
            style={{
        ...
        {TOOLS.map((tool) => (
          <div
            ...
            style={{
              ...
              textAlign: 'center',
              cursor: 'pointer',
              background: selectedTools.includes(tool.value) ? colors.primary[50] : colors.neutral[0],
              transition: 'all 150ms',
            }}
          >
            <div style={{ fontSize: '2rem', marginBottom: spacing.sm }}>{tool.icon}</div>
            <div style={{ fontSize: typography.fontSize.sm, fontWeight: typography.fontWeight.medium }}>{tool.label}</div>
          </div>
        ))}
      </div>
    </div>
...
We&apos;ll enable stricter logging and redaction
        ...
        >
          <div style={{ fontSize: '3rem', marginBottom: spacing.md }}>🌐</div>
          <div style={{ fontSize: typography.fontSize.lg, fontWeight: typography.fontWeight.semibold }}>No</div>
          <p style={{ fontSize: typography.fontSize.sm, color: colors.neutral[500], marginTop: spacing.sm }}>
  ...
  return (
    <div>
      <div style={{ textAlign: 'center', marginBottom: spacing['2xl'] }}>
        <div style={{ fontSize: '3rem', marginBottom: spacing.md }}>✨</div>
        <h2 style={{ fontSize: typography.fontSize['3xl'], fontWeight: typography.fontWeight.bold, marginBottom: spacing.md }}>
      ...
      <Card style={{ background: colors.primary[50], border: `2px solid ${colors.primary[200]}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: spacing.lg, marginBottom: spacing.lg }}>
          ...
          <div>
            ...
            <p style={{ fontSize: typography.fontSize.sm, color: colors.neutral[600] }}>{pack.description}</p>
          </div>
        </div>

        <div style={{ marginTop: spacing.xl }}>
          <p style={{ fontSize: typography.fontSize.sm, fontWeight: typography.fontWeight.semibold, marginBottom: spacing.md }}>
            Included agents:
          </p>
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {pack.agents.map((agent) => (
              <li key={agent.id} style={{ marginBottom: spacing.md, display: 'flex', gap: spacing.sm }}>
                <span>✓</span>
                <div>
                  <div style={{ fontWeight: typography.fontWeight.medium }}>{agent.name}</div>
                  <div style={{ fontSize: typography.fontSize.sm, color: colors.neutral[600] }}>{agent.description}</div>
                </div>
              </li>
            ))}
          </ul>
      ...
      <div style={{ marginTop: spacing.lg, fontSize: typography.fontSize.sm, color: colors.neutral[500], textAlign: 'center' }}>
