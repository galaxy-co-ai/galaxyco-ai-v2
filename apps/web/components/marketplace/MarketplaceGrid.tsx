    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-base mb-8 max-w-md mx-auto" style={{ color: 'var(--text-secondary)' }}>
          Be the first to create and share an agent with the community. Build custom agents for your
          workflows and publish them here.
        </p>
      ...
      <div>
        ...
        >
          {allTemplates.slice(0, 4).map((template /* Show 4 featured instead of 3 */) => (
            <AgentTemplateCardCompact key={template.id} template={template} isFeatured={true} />
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        >
          {allTemplates.slice(4).map((template /* Start from 4 since featured shows 4 */) => (
            <AgentTemplateCardCompact key={template.id} template={template} isFeatured={false} />
          ))}
        </div>
      </div>
    </div>
