  ...
  if (allTemplates.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        >
          Be the first to create and share an agent with the community. Build
          custom agents for your workflows and publish them here.
        </p>
      ...
      <div>
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns:
              "repeat(4, 1fr)" /* 4 cards per row for density */,
        ...
        >
          {allTemplates
            .slice(0, 4)
            .map((template /* Show 4 featured instead of 3 */) => (
      ...
      <div>
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns:
              "repeat(5, 1fr)" /* 5 cards per row for maximum density */,
            gap: "var(--grid-gap)" /* 18px - compact spacing */,
          }}
          className="grid-compact" /* Apply responsive grid from design tokens */
        >
          {allTemplates
            .slice(4)
            .map((template /* Start from 4 since featured shows 4 */) => (
              <AgentTemplateCardCompact
                key={template.id}
                template={template}
                isFeatured={false}
