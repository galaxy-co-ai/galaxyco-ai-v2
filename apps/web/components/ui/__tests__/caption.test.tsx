...
describe('Caption', () => {
  ...
  it('renders with different sizes', () => {
    const { container: cXs } = render(React.createElement(Caption, { size: 'xs' }, 'Extra Small'));
    const { container: cSm } = render(React.createElement(Caption, { size: 'sm' }, 'Small'));
  ...
  it('muted variant has muted color', () => {
    const { container } = render(React.createElement(Caption, { variant: 'muted' }, 'Muted text'));
    expect(container.firstChild).toHaveClass('text-foreground-muted');
  });

  it('error variant has destructive color', () => {
    const { container } = render(React.createElement(Caption, { variant: 'error' }, 'Error text'));
