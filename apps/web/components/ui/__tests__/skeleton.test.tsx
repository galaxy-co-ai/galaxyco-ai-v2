...
describe("Skeleton", () => {
  ...
  it("renders with different variants", () => {
    ...
    const { container: containerText } = render(
  ...
  it("renders with different sizes", () => {
    ...
    const { container: containerLg } = render(
      React.createElement(Skeleton, { size: "lg" }),
    );
    const { container: containerXl } = render(
      React.createElement(Skeleton, { size: "xl" }),
    );
  ...
  it("applies custom className", () => {
    const { container } = render(
      React.createElement(Skeleton, { className: "w-full h-20" }),
    );
    const skeleton = container.firstChild as HTMLElement;
  ...
  it("supports additional HTML attributes", () => {
    const { container } = render(
      React.createElement(Skeleton, { id: "loading-skeleton" }),
    );
    const skeleton = container.firstChild as HTMLElement;
