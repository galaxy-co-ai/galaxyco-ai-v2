...
describe('Input', () => {
  ...
  it('renders with destructive variant', () => {
    const { container } = render(React.createElement(Input, { variant: 'destructive' }));
    const input = container.querySelector('input');
    expect(input).toHaveClass('border-destructive');
  });

  it('renders with success variant', () => {
    const { container } = render(React.createElement(Input, { variant: 'success' }));
    const input = container.querySelector('input');
    expect(input).toHaveClass('border-success');
  });

  it('applies destructive variant when error prop is provided', () => {
    const { container } = render(React.createElement(Input, { error: 'This field is required' }));
    const input = container.querySelector('input');
    expect(input).toHaveClass('border-destructive');
  ...
  it('can be read-only', () => {
    render(React.createElement(Input, { readOnly: true, value: 'Read only text' }));
  ...
  it('accepts different input types', () => {
    const { container: emailContainer } = render(React.createElement(Input, { type: 'email' }));
