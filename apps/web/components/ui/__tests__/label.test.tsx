...
describe('Label', () => {
  ...
  it('shows asterisk when required is true', () => {
    const { container } = render(React.createElement(Label, { required: true }, 'Required Field'));
  ...
  it('does not show asterisk when required is false', () => {
    const { container } = render(React.createElement(Label, { required: false }, 'Optional Field'));
    const label = container.querySelector('label');
    expect(label).not.toHaveClass("after:content-['*']");
  });

  it('applies htmlFor attribute correctly', () => {
    render(React.createElement(Label, { htmlFor: 'test-input' }, 'Label for input'));
