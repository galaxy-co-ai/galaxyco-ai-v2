...
describe('Spinner', () => {
  ...
  it('renders with custom label', () => {
    const { container } = render(React.createElement(Spinner, { label: 'Fetching data...' }));
    const spinner = container.firstChild as HTMLElement;
  ...
  it('renders with different variants', () => {
    ...
    expect(containerDestructive.firstChild).toHaveClass('border-destructive/20');
  });

  it('renders with different sizes', () => {
    const { container: containerXs } = render(React.createElement(Spinner, { size: 'xs' }));
    const { container: containerSm } = render(React.createElement(Spinner, { size: 'sm' }));
    const { container: containerLg } = render(React.createElement(Spinner, { size: 'lg' }));
    const { container: containerXl } = render(React.createElement(Spinner, { size: 'xl' }));
  ...
  it('applies custom className', () => {
    const { container } = render(React.createElement(Spinner, { className: 'custom-spinner' }));
    const spinner = container.firstChild as HTMLElement;
  ...
  it('renders screen reader text', () => {
    const { container } = render(React.createElement(Spinner, { label: 'Loading data' }));
