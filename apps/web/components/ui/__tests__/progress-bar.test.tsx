...
describe('ProgressBar', () => {
  ...
  it('renders with different variants', () => {
    ...
    expect(cDefault.querySelector('[role="progressbar"]')).toHaveClass('bg-muted');
    expect(cPrimary.querySelector('[role="progressbar"]')).toHaveClass('bg-primary/20');
    expect(cSuccess.querySelector('[role="progressbar"]')).toHaveClass('bg-success/20');
    expect(cWarning.querySelector('[role="progressbar"]')).toHaveClass('bg-warning/20');
    expect(cDestructive.querySelector('[role="progressbar"]')).toHaveClass('bg-destructive/20');
  });

  it('renders with different sizes', () => {
    const { container: cSm } = render(React.createElement(ProgressBar, { size: 'sm' }));
    const { container: cDefault } = render(React.createElement(ProgressBar, { size: 'default' }));
    const { container: cLg } = render(React.createElement(ProgressBar, { size: 'lg' }));
  ...
  it('renders with striped appearance', () => {
    const { container } = render(React.createElement(ProgressBar, { striped: true }));
    const progressbar = container.querySelector('[role="progressbar"]');
  ...
  it('does not animate without striped', () => {
    const { container } = render(React.createElement(ProgressBar, { animated: true }));
    const progressbar = container.querySelector('[role="progressbar"]');
    const fill = progressbar?.querySelector('div');
  ...
  it('renders indeterminate state', () => {
    const { container } = render(React.createElement(ProgressBar, { indeterminate: true }));
    const progressbar = container.querySelector('[role="progressbar"]');
  ...
  it('respects max value', () => {
    const { container } = render(React.createElement(ProgressBar, { value: 50, max: 200 }));
    const progressbar = container.querySelector('[role="progressbar"]');
  ...
  it('has rounded-full class', () => {
    const { container } = render(React.createElement(ProgressBar));
    expect(container.querySelector('[role="progressbar"]')).toHaveClass('rounded-full');
  });
