...
describe('LoadingSkeletons', () => {
  ...
  it('renders AgentGridSkeleton with custom count', () => {
    const { container } = render(React.createElement(AgentGridSkeleton, { count: 3 }));
    expect(container.querySelector('[class*="grid"]')).toBeInTheDocument();
  });
  ...
  it('renders KnowledgeGridSkeleton with custom count', () => {
    const { container } = render(React.createElement(KnowledgeGridSkeleton, { count: 12 }));
    expect(container.querySelector('[class*="grid"]')).toBeInTheDocument();
  });
  ...
  it('renders MarketplaceGridSkeleton with custom count', () => {
    const { container } = render(React.createElement(MarketplaceGridSkeleton, { count: 6 }));
    expect(container.querySelector('[class*="grid"]')).toBeInTheDocument();
  });
  ...
  it('renders TableRowSkeleton with custom columns', () => {
    const { container } = render(React.createElement(TableRowSkeleton, { columns: 6 }));
    expect(container.firstChild).toBeInTheDocument();
  });
