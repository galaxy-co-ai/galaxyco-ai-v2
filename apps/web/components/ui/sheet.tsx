...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-overlay bg-background-overlay backdrop-blur-sm",
...
const sheetVariants = cva(
  "fixed z-drawer gap-spacing-lg bg-background p-spacing-lg shadow-elevation-high border border-border transition ease-in-out",
  {
    variants: {
      side: {
...
const SheetHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col gap-spacing-xs text-center sm:text-left mb-spacing-md",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
className={cn("text-heading-lg font-heading text-foreground", className)}
    {...props}
  />
))
