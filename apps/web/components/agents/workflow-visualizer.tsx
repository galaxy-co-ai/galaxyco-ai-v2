...
import type {
  WorkflowNode as StoreWorkflowNode,
  WorkflowEdge,
...
function CustomNode({ data }: { data: any }) {
  ...
  const getBgColor = () => {
    ...
    if (type === 'condition')
      return 'bg-amber-100 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300 border-amber-300 dark:border-amber-700';
    if (status === 'active')
      return 'bg-blue-100 dark:bg-blue-900/20 border-blue-500 dark:border-blue-400';
    if (status === 'complete')
      return 'bg-green-50 dark:bg-green-900/10 border-green-400 dark:border-green-600';
    if (status === 'error')
      return 'bg-red-50 dark:bg-red-900/10 border-red-400 dark:border-red-600';
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          ...
          {integration && <div className="mt-1 text-xs opacity-70">{integration}</div>}
        </div>
      </div>
    </div>
  );
