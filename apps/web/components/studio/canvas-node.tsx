...
const NODE_ICONS: Record<GridNodeType, React.ComponentType<{ className?: string }>> = {
  trigger: Zap,
  action: Play,
  condition: GitBranch,
  loop: Repeat,
...
const NODE_COLORS: Record<GridNodeType, { bg: string; border: string; icon: string }> = {
  trigger: {
      ...
      <div className="p-3 space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <h4 className="text-sm font-semibold truncate text-foreground">{data.label}</h4>
          </div>

          {/* Status Indicator */}
          <div
            className={cn('w-2 h-2 rounded-full', statusStyle.bg, statusStyle.ring, 'ring-2')}
            title={status}
          />
        </div>
