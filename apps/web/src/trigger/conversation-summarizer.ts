...
export const generateConversationTitleTask = task({
  ...
  run: async (payload: GenerateTitlePayload, { ctx }) => {
    ...
    try {
      ...
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "Generate a short, descriptive title (max 50 chars) for this conversation. Return only the title, no quotes.",
            },
            {
              role: "user",
      ...
      const title = generatedTitle.length > 50 ? generatedTitle.slice(0, 47) + "..." : generatedTitle;
      ...
      const tagsRes = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "Extract 3-5 key topics/tags from this conversation. Return as JSON array of strings.",
            },
            {
              role: "user",
