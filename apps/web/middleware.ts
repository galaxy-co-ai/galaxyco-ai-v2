...
export default clerkMiddleware(async (auth, request) => {
  ...
  if (ROUTE_REDIRECTS[pathname]) {
    const redirectUrl = new URL(ROUTE_REDIRECTS[pathname] + search, request.url);
  ...
  if (pathname.startsWith('/integrations/') && !pathname.startsWith('/automations/')) {
    const redirectUrl = new URL(
  ...
  if (pathname.startsWith('/webhooks/') && !pathname.startsWith('/developer/')) {
    const redirectUrl = new URL(
