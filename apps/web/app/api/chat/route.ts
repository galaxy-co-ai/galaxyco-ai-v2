...
import { users, workspaceMembers, chatMessages } from '@galaxyco/database/schema';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!rateLimitResult.success) {
      logger.warn('Chat creation rate limit exceeded', {
        userId: clerkUserId,
    ...
    const validation = safeValidateRequest(createChatMessageSchema, body);
    ...
    const [message] = await db.insert(chatMessages).values(insertValues).returning();
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { userId: clerkUserId } = await auth();
    if (!clerkUserId) {
      logger.warn('Unauthorized chat list request');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
