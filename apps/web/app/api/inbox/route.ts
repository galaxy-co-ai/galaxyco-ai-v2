...
import { users, workspaceMembers, inboxMessages } from '@galaxyco/database/schema';
import { eq, and, desc } from 'drizzle-orm';
import { createInboxMessageSchema } from '@/lib/validation/communication';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!clerkUserId) {
      logger.warn('Unauthorized message creation attempt');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    // 2. Rate limiting check
    const rateLimitResult = await checkRateLimit(clerkUserId, RATE_LIMITS.CHAT_MESSAGE);
    if (!rateLimitResult.success) {
      logger.warn('Inbox creation rate limit exceeded', {
        userId: clerkUserId,
    ...
    const validation = safeValidateRequest(createInboxMessageSchema, body);

    if (!validation.success) {
      const formattedError = formatValidationError(validation.error);
      logger.warn('Invalid message creation request', {
    ...
    const { workspaceId, ...messageData } = validation.data;
    ...
    const insertValues: typeof inboxMessages.$inferInsert = {
      workspaceId,
      channel: messageData.channel,
      subject: messageData.subject,
      body: messageData.body,
      recipientIds: messageData.recipientIds as string[],
      senderId: user.id,
      status: 'unread',
    };

    const [message] = await db.insert(inboxMessages).values(insertValues).returning();
...
/**
 * GET /api/inbox
 * List all inbox for a workspace
 *
 * Query params:
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { userId: clerkUserId } = await auth();
    if (!clerkUserId) {
      logger.warn('Unauthorized inbox list request');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    ...
    // 5. Fetch inbox messages from database
