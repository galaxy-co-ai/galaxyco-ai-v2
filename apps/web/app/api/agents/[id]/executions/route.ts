...
import { agentExecutions, agents, workspaceMembers, users } from '@galaxyco/database/schema';
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const stats = await db
      .select({
        total: count(),
        completed: count(sql`CASE WHEN ${agentExecutions.status} = 'completed' THEN 1 END`),
        failed: count(sql`CASE WHEN ${agentExecutions.status} = 'failed' THEN 1 END`),
        pending: count(sql`CASE WHEN ${agentExecutions.status} = 'pending' THEN 1 END`),
        running: count(sql`CASE WHEN ${agentExecutions.status} = 'running' THEN 1 END`),
