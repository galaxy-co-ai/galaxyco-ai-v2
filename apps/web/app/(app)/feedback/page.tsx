              ...
              <div key={feedback.id} className="rounded-lg border border-border bg-card p-4">
                <div className="flex items-start gap-4">
                  ...
                  <button
                    ...
                    aria-label={userVotes[feedback.id] ? 'Remove vote' : 'Vote for this feature'}
                  >
                    <ThumbsUp className={cn('h-5 w-5', userVotes[feedback.id] && 'fill-current')} />
                    <span className="text-sm font-semibold">
                      {feedback.votes +
                        (userVotes[feedback.id] && !feedback.userVoted ? 1 : 0) +
                        (feedback.userVoted && !userVotes[feedback.id] ? -1 : 0)}
                    </span>
                  </button>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      <Badge
                        variant="secondary"
                        className={
                          statusConfig[feedback.status as keyof typeof statusConfig].className
                        }
                      >
