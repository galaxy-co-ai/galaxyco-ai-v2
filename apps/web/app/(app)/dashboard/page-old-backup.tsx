    ...
    <div className="bg-white rounded-2xl border-2 border-gray-300 p-6 h-full flex flex-col">
      <div className="space-y-2">
        {sidebarCategories.map((category) => {
          ...
          const isSelected =
            selectedCategory === category.id || category.isActive;
...
function MainChart({ selectedCategory }: { selectedCategory: string | null }) {
  const chartData =
    categoryChartData[selectedCategory || "user-time"] ||
    categoryChartData["user-time"];
  const chartConfig =
    categoryChartConfig[selectedCategory || "user-time"] ||
    categoryChartConfig["user-time"];

  return (
    <div className="bg-white rounded-2xl border-2 border-gray-300 p-6 flex-1">
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">
          {sidebarCategories.find((c) => c.id === selectedCategory)?.title ||
            "User Time"}{" "}
          Analytics
        </h3>
