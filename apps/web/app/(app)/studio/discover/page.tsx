...
export default function StudioDiscoverPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/studio/templates?workspaceId=${currentWorkspace.id}`);
  ...
  const filteredTemplates = useMemo(() => {
    return templates.filter((template: GridTemplate) => {
      ...
      if (searchQuery) {
        ...
        const matchesTags = template.tags?.some((tag: string) => tag.toLowerCase().includes(query));
    ...
    <>
      ...
      {!isLoading && templates.length > 0 && (
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <MetricCard label="Total Templates" value={metrics.total} icon={Sparkles} />
          <MetricCard label="Featured" value={metrics.featured} icon={Star} />
          <MetricCard label="Total Uses" value={metrics.totalUses.toLocaleString()} icon={Users} />
          <MetricCard label="Avg Rating" value={metrics.avgRating} icon={TrendingUp} />
        </div>
      )}
      ...
      >
        {filteredTemplates.length === 0 && templates.length > 0 ? (
          <div className="text-center py-12">
            <p className="text-muted-foreground mb-4">No templates match your search or filters</p>
