...
function CampaignCard({ campaign }: { campaign: (typeof outreachCampaigns)[0] }) {
  ...
  return (
    <Card className="p-6 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-2 mb-4">
        ...
        <Progress
          value={campaign.sent > 0 ? (campaign.replied / campaign.sent) * 100 : 0}
          className="h-2"
        />
      </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        <div className="flex items-center gap-2">
          <Avatar
            ...
            size="xs"
          />
          <span className="text-sm text-muted-foreground">{campaign.owner.name}</span>
        </div>
...
export default function OutreachPage() {
  ...
  const filteredCampaigns = outreachCampaigns.filter((campaign) => {
    ...
    const matchesStatus =
      !activeFilters.status?.length || activeFilters.status.includes(campaign.status);

    const matchesType = !activeFilters.type?.length || activeFilters.type.includes(campaign.type);
  ...
  const filters = [
    {
      ...
      options: [
        ...
        },
        {
          value: 'completed',
          label: 'Completed',
          count: outreachCampaigns.filter((c) => c.status === 'completed').length,
        },
      ],
    },
    {
      ...
      options: [
        {
          ...
          count: outreachCampaigns.filter((c) => c.type === 'Email Sequence').length,
        },
        {
          value: 'Phone Sequence',
          label: 'Phone Sequence',
          count: outreachCampaigns.filter((c) => c.type === 'Phone Sequence').length,
        },
        {
          value: 'Mixed Sequence',
          label: 'Mixed Sequence',
          count: outreachCampaigns.filter((c) => c.type === 'Mixed Sequence').length,
        },
        {
          value: 'Social Sequence',
          label: 'Social Sequence',
          count: outreachCampaigns.filter((c) => c.type === 'Social Sequence').length,
        },
      ],
    },
  ];
