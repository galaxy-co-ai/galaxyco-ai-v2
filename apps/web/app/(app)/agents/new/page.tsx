...
import {
  TemplateGallery,
  type Template,
} from "@/components/agents/template-gallery";
...
export default function AgentBuilderPage() {
  ...
  const [progressSteps, setProgressSteps] = useState<
    Array<{ label: string; status: "pending" | "active" | "complete" }>
  >([]);
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Failed to generate agent variants";
      toast.error(message);
      setProgressSteps([]);
    } finally {
  ...
  const handleRunTest = async (inputs: any) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Test completed! All ${result.executedSteps} steps passed.`,
        );
      } else {
        ...
        {currentStep === "iteration" && selectedVariant && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-neutral-600 dark:text-neutral-400">
              Use the chat to describe changes. The workflow updates in
              real-time.
            </p>
