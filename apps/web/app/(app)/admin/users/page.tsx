...
export default function AdminUsersPage() {
  ...
  async function handleUpdateUser(updatedData: Partial<AdminUser>) {
    ...
    try {
      ...
      const offset = (page - 1) * limit;
      const usersRes = await fetch(`/api/admin/users?limit=${limit}&offset=${offset}`);
      if (usersRes.ok) {
  ...
  const rows = useMemo(() => {
    return users.map((u) => ({
      ...
      lastActive: u.lastLoginAt ? new Date(u.lastLoginAt).toLocaleString() : '—',
                  ...
                  <td className="px-4 py-3 text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem
                          onClick={() => setEditUser(users.find((u) => u.id === user.id) || null)}
                        >
                          <Edit className="mr-2 h-4 w-4" />
                          Edit User
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => setDeleteUser(users.find((u) => u.id === user.id) || null)}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">
                    {selectedUser.workspaceMembers?.[0]?.isActive ? 'Active' : 'Inactive'}
                  </p>
                </div>
              </div>
            </div>
