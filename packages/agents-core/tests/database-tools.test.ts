...
describe("search_agents Tool", () => {
  ...
  beforeEach(() => {
    ...
    withTenant.mockImplementation((db, workspaceId) =>
      createMockDb(workspaceId),
    );
  });
  ...
  it("should search for agents in workspace-1", async () => {
    ...
    const result = await searchAgentsTool.execute(
...
describe("get_workspace_stats Tool", () => {
  ...
  it("should require workspace context", async () => {
    ...
    const result = await getWorkspaceStatsTool.execute(
      {},
      contextWithoutWorkspace,
    );
...
describe("Multi-Tenant Security", () => {
  ...
  it("should enforce tenant isolation across all tools", async () => {
    ...
    expect(search1.agents.every((a: any) => a.id.startsWith("agent-1-"))).toBe(
      true,
    );

    // Workspace 2 should only see its own data
    const search2 = await searchTool.execute({ query: "" }, context2);
    expect(search2.agents.every((a: any) => a.id.startsWith("agent-2-"))).toBe(
      true,
    );

    // Cross-tenant agent access should fail
    const crossAccess1 = await getTool.execute(
      { agentId: "agent-2-1" },
      context1,
    );
    expect(crossAccess1.success).toBe(false);

    const crossAccess2 = await getTool.execute(
      { agentId: "agent-1-1" },
      context2,
    );
