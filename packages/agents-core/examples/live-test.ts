...
const calculatorTool = createTool(
  ...
  async (args: { operation: string; a: number; b: number }) => {
    ...
    console.log(`  🔧 TOOL CALLED: calculate(${operation}, ${a}, ${b}) = ${result}`);
...
Always show your work and explain the result.`,
  ...
  console.log('Tools:', toolAgent.tools.map((t) => t.definition.function.name).join(', '));
  console.log('Making API call with tool access...\n');

  const result2 = await Runner.run(toolAgent, [{ role: 'user', content: 'What is 47 times 23?' }]);
  ...
  const result3 = await Runner.run(toolAgent, [
    {
      role: 'user',
      content: 'I have 150 dollars. I buy 3 items at 23 dollars each. How much do I have left?',
    },
  ]);
