...
test.describe("Document Upload Flow", () => {
  ...
  test("can navigate to knowledge/documents page", async ({ page }) => {
    ...
    const knowledgeLink = page
      .locator('a[href*="/knowledge"], a[href*="/documents"]')
      .first();
    ...
    const url = page.url();
    expect(
      url.includes("/knowledge") || url.includes("/documents"),
    ).toBeTruthy();
  });
  ...
  test("validates file size limits", async ({ page }) => {
    ...
    if ((await fileInput.count()) > 0) {
      ...
      const sizeError = page.locator(
        'text=/too large/i, text=/file size/i, [role="alert"]',
      );
...
test.describe("Document List Updates", () => {
  ...
  test("document list updates after successful upload", async ({ page }) => {
    ...
    if ((await fileInput.count()) > 0) {
      ...
      expect(
        newCount >= initialCount || appearsInList ? true : false,
      ).toBeTruthy();
    }
  });
});
