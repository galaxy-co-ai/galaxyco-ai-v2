...
import { getCurrentTenantContext, withTenantFilter, validateTenantAccess } from '../../apps/web/lib/db/tenant-filter';
...
describe('Tenant Isolation Security Tests', () => {
  ...
  test('tenant-filtered queries should only return tenant data', async () => {
    ...
    const tenant1Agents = await db.select()
      .from(agents)
    ...
    const tenant2Agents = await db.select()
      .from(agents)
  ...
  test('cross-tenant resource access should be blocked', async () => {
    ...
    const attemptedAccess = await db.select()
      .from(agents)
      .where(and(
        eq(agents.id, agent2Id),
        withTenantFilter(tenant1Id, agents.workspaceId)
      ));
  ...
  test('workspace membership isolation', async () => {
    ...
    const user1Memberships = await db.select()
    ...
    const user2Memberships = await db.select()
      .from(workspaceMembers)
...
describe('Row-Level Security Policy Tests', () => {
  ...
  test('tenant isolation policies should exist', async () => {
    ...
    const expectedTables = ['agents', 'workspace_members', 'workflows', 'executions', 'knowledge_items'];
