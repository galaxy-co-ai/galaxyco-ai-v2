...
const SAMPLE_AGENTS = [
  {
    ...
    config: {
      ...
      temperature: 0.7,
      maxTokens: 1000,
      systemPrompt: 'You are a helpful customer support agent. Be friendly, professional, and solution-oriented.',
      tools: [],
  ...
  },
  {
    ...
    config: {
      ...
      temperature: 0.5,
      maxTokens: 800,
      systemPrompt: 'You are a sales qualification agent. Ask relevant questions to assess lead quality and provide a qualification score.',
      tools: [],
  ...
  },
  {
    ...
    config: {
      ...
      temperature: 0.3,
      maxTokens: 2000,
      systemPrompt: 'You are a content summarization expert. Extract key points and create concise, actionable summaries.',
      tools: [],
      triggers: [{ type: 'manual', config: {} }],
      knowledgeBase: {
        enabled: false,
  ...
  },
  {
    ...
    config: {
      ...
      temperature: 0.4,
      maxTokens: 1500,
      systemPrompt: 'You generate structured meeting notes. Include key decisions, action items, and next steps.',
      tools: [],
  ...
  },
  {
    ...
    config: {
      ...
      temperature: 0.2,
      maxTokens: 2000,
      systemPrompt: 'You are a code review expert. Analyze code for bugs, performance issues, and best practices.',
      tools: [],
    ...
    for (const collectionData of SAMPLE_COLLECTIONS) {
      ...
      const existingCollection = await db.query.knowledgeCollections.findFirst({
        where: (collections, { and, eq }) => and(
          ...
          eq(collections.name, collectionData.name)
        ),
      });
      ...
      if (existingCollection) {
        ...
        const [collection] = await db.insert(knowledgeCollections).values({
          ...collectionData,
          workspaceId: workspace.id,
          createdBy: owner.id,
        }).returning();
